...
def estimate(req: ImageRequest):
    try:
        ...
        if resp.status_code != 200:
            raise HTTPException(status_code=400, detail=f"Could not download image. Status {resp.status_code}")
        ...
        if len(xs) == 0 or len(ys) == 0:
            raise HTTPException(status_code=400, detail="No bright sign pixels detected.")
        ...
        if sign_width_px <= 0:
            raise HTTPException(status_code=400, detail="Detected sign width was zero.")
